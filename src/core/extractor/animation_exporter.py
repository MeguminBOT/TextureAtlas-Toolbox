...
def _build_exif_comment(comment: str) -> Optional[bytes]:
    """Build minimal EXIF bytes with an ImageDescription tag.

    Args:
        comment: Text to embed as ImageDescription.

    Returns:
        EXIF bytes suitable for Pillow's ``exif`` parameter, or None on failure.
    """
    try:
